...
const usagenote = "При обычном использовании в протоколе TCPMail на сервере отобразится подключение от клиента (123456 по умолчанию), получено: 0, оптравлено: 0.\nПри запуске с флагом -M (мониторинг) на сервере отобразится 'Сервер. Мониторинг'.\nПротокол записывается в текущую директорию (которая в консоле открыта при запуске утилиты)."

func main() {
	q, wait, step, addr, ccode := promptVariables()
	logger := log.New(os.Stdout, "", log.LstdFlags)
	logfile, err := os.Create("tcpmtest_" + time.Now().Format("060102-150405") + ".log")
	defer func() {
